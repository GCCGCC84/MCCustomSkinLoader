...
The **Game Test** workflow (`.github/workflows/game-test.yml`) runs on every push and pull request, and can also be dispatched manually. It builds the Universal jar, prepares one shared download cache (reused read-only by the test jobs), then runs one job per Minecraft version. Each job starts a vanilla server once and tests every loader available for that version in sequence. Results (screenshots, `latest.log`, `CustomSkinLoader.log`, client/server output and `result.json`) are uploaded as artifacts and summarized in the run's job summary. The manual `game_versions` and `loaders` inputs restrict a dispatch run to a focused subset. Two compatibility shims keep the older versions deterministic: Fabric/Quilt clients on 1.14-1.16.5 connect through a local join relay that holds world packets until the first resource reload finishes (the vanilla early `--server` connect race, MC-145102), and 1.16.4/1.16.5 launch with a dead local proxy so the offline session does not silently disable `--server` auto-connect.
