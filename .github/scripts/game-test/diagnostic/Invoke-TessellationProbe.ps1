...
$result = [ordered]@{
    ...
    withCsl = $WithCsl; deferredJoin = $DeferredJoin; status = 'setup-failed'
    ...
    exitCode = $null; error = $null; durationSeconds = 0
}
...
try {
    ...
    @('guiScale:2', 'lang:en_us', 'skipMultiplayerWarning:true') | Set-Content (Join-Path $gameDir 'options.txt') -Encoding ascii
    if ($WithCsl) {
    ...
    $launch = New-MinecraftLaunchArguments -Profile $profile -Runtime $runtime -JavaExe $javaExe -GameDir $gameDir -Username 'CslTest'
    if ($DeferredJoin) {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($all -match 'Description: Rendering screen') { $result.renderingCrash = $true }
        ...
        if ($result.serverJoined -and ($watch.Elapsed.TotalSeconds - $joinAt) -ge $ObserveSeconds) {
            $result.status = if ($result.clientWorldLoaded) { 'stable-world' } else { 'joined-without-world' }
            break
        }
        Start-Sleep -Seconds 1
    }
    if ($result.status -eq 'setup-failed') { $result.status = 'join-timeout' }
